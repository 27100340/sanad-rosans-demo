...
import type { GuardEventType, GuardMode } from "@/lib/domain/proctor";
 ...
 *              (hidden / blur / split-screen) cancels the attempt. Screenshots
 *              are deterred with a black-out. Every trip is reported via onEvent.
...
const ARM_DELAY_MS = 1400;
const BLACKOUT_MS = 1000;
const BLACKOUT_CLASS = "el-blackout";
const CLIPBOARD_NOTICE = "Screenshots are disabled during a test.";
...
export function useExamGuard({
  ...
  active: boolean;
  mode?: GuardMode;
  onViolation: (reason: string, ev: GuardEvent) => void;
  ...
  useEffect(() => {
    ...
    const onVis = () => {
      if (document.hidden) report("hidden", "You minimised or switched away from the test screen.", true);
    };
    const onBlur = () => {
      if (fieldFocused()) return;
      report("blur", "You left the test window (alt-tab / split-screen / another app).", true);
    };
    const onFocus = () => {
      // Non-terminal: useful in the forensic timeline to see how long they were away.
      if (strict) report("focus", "Returned to the test window.", false);
    };
    const onResize = () => {
      let b = base.current;
      if (!b.w) return;
    ...
    const blackout = () => {
      document.documentElement.classList.add(BLACKOUT_CLASS);
      setTimeout(() => document.documentElement.classList.remove(BLACKOUT_CLASS), BLACKOUT_MS);
    };
    const onKey = (e: KeyboardEvent) => {
      const k = e.key;
      const snip = k === "PrintScreen" || (e.metaKey && e.shiftKey && ["3", "4", "5", "S", "s"].includes(k)) || (e.ctrlKey && (k === "p" || k === "P"));
      if (snip) {
        try {
          navigator.clipboard?.writeText?.(CLIPBOARD_NOTICE);
        } catch {
          /* ignore */
        }
        blackout();
        ...
        // Strict tests cancel on a capture attempt; standard tests only deter + log.
    ...
    const preventCtx = (e: Event) => {
      e.preventDefault();
      report("contextmenu", "You opened the right-click menu.", false);
    };
    const preventDrag = (e: Event) => e.preventDefault();
    const onFsChange = () => {
      if (strict && !document.fullscreenElement) {
        report("fullscreen_exit", "You left full-screen test mode.", true);
      }
    };
    ...
    return () => {
      ...
      document.documentElement.classList.remove(BLACKOUT_CLASS);
    };
  }, [active, mode]);
}
