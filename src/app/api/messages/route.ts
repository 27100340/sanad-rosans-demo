 ...
 * student, optionally to their guardians; a show-cause notice always reaches
 * guardians and is queued as an email. Anyone GETs their own notifications
 * and PATCHes them read. Notifications are keyed by the person id: the
 * student id for students, the guardian id for parents, the staff id otherwise.
 */
...
export async function POST(req: Request) {
  ...
  const allowed = new Set(spacesForTeacher(viewer.personId).map((s) => s.classId));
  ...
  if (!classId || !allowed.has(classId)) return Response.json({ error: "That class is not one of your spaces." }, { status: 403 });
