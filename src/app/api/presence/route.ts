...
export async function GET() {
  ...
  else if (viewer.role === "principal" || viewer.role === "chairman") ids = [...STUDENTS.filter((s) => viewer.branchId === null || s.branchId === viewer.branchId).map((s) => s.id), ...TEACHERS.filter((t) => viewer.branchId === null || t.branchId === viewer.branchId).map((t) => t.id)];
