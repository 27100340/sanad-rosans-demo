import { LESSON_STATUS } from "@/components/attend/lesson-list";
import { Register, type RegisterRow } from "@/components/attend/register";
import { canSeePrincipal } from "@/components/leadership/seat-guard";
import { Denied } from "@/components/teach/guard";
import { fmtDay } from "@/components/teach/helpers";
import { Chip, LinkButton, PageHeader } from "@/components/ui/primitives";
import { getViewer } from "@/lib/auth/viewer";
import { lessonById, marksForLesson } from "@/lib/data/mock/attendance";
import { classById, studentsInClass } from "@/lib/data/mock/people";

export default async function RegisterPage({ params }: { params: Promise<{ lessonId: string }> }) {
  const { lessonId } = await params;
  const viewer = await getViewer();
  const lesson = lessonById.get(lessonId);
  const owns = Boolean(lesson) && viewer.role === "teacher" && lesson?.teacherId === viewer.personId;
  if (!lesson || (!owns && !canSeePrincipal(viewer))) return <Denied />;

  const className = classById.get(lesson.classId)?.name ?? lesson.classId;
  const marks = new Map(marksForLesson(lesson.id).map((m) => [m.studentId, m]));
  const rows: RegisterRow[] = studentsInClass(lesson.classId).map((s) => ({
    studentId: s.id,
    name: s.name,
    status: marks.get(s.id)?.status ?? null,
    note: marks.get(s.id)?.note ?? "",
  }));
  const st = LESSON_STATUS[lesson.status];

  return (
    <>
      <PageHeader
        eyebrow={`${lesson.subject} · ${className} · Period ${lesson.period} · ${fmtDay(lesson.date)}`}
        title="Register"
        description={`Room ${lesson.room} · ${rows.length} student${rows.length === 1 ? "" : "s"} · ${st.label.toLowerCase()}. Tap a status to move it to the next one.`}
        actions={
          <>
            <Chip tone={st.tone}>{st.label}</Chip>
            {owns ? <LinkButton href="/portal/teach/attendance" variant="ghost" className="btn-sm">All lessons</LinkButton> : null}
          </>
        }
      />
      <Register lessonId={lesson.id} status={lesson.status} rows={rows} canEdit={owns} />
    </>
  );
}
