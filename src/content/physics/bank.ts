/**
 * Authored CAIE 9702 practice questions, ported verbatim from the Physics
 * Studio Exam Lab seed bank written by the subject educator. AS and A2 topics,
 * LOT/HOT thinking levels, Paper 1 / 2 / 4, Cambridge command words and real
 * mark schemes.
 *
 * Stems and mark schemes are authored in KaTeX. Sanad renders markdown WITHOUT
 * KaTeX, so nothing here reaches a page raw: `PHYSICS_BANK` below runs every
 * stem, option and mark-scheme line through `toPlainPhysics` at module load.
 * The LaTeX is kept in the source so the port stays faithful and re-portable.
 *
 * AUTO-PORTED from sjabrankamran-site/src/lib/exam-lab/bank.ts — regenerate
 * rather than editing by hand.
 */
import type { PhysicsQuestion, RawPhysicsQuestion } from "@/lib/domain/physics";
import { plainQuestion } from "./latex";

/** The 9702 syllabus topic map, as the Exam Lab tags questions. */
export const TOPICS = {
  AS: [
...
export const AS_TOPICS: string[] = TOPICS.AS;
export const A2_TOPICS: string[] = TOPICS.A2;
export const ALL_TOPICS: string[] = [...TOPICS.AS, ...TOPICS.A2];

const RAW: RawPhysicsQuestion[] = [
...
/** Authored questions, id-stamped and converted to plain Unicode physics text. */
export const AUTHORED_BANK: PhysicsQuestion[] = RAW.map((q, i) =>
  plainQuestion({
